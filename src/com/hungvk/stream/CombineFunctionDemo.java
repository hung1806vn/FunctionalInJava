...
public class CombineFunctionDemo {
	...
	public static void main(String[] args) {
		...
		Float total = employees.stream()
				.filter(x->"developer".equals(x.jobTitle))
				.map(x->x.salary)
				.reduce(0f, (acc,x)->acc+x);
		
		Long numDevs = employees.stream().filter(x->"developer".equals(x.jobTitle)).collect(Collectors.counting());
		
		Float ava = total/numDevs;
